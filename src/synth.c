...
int32_t synth_next_sample(Voice *voice) {
    ...
    uint32_t index = voice->phase >> WAVETABLE_SHIFT;  // 0..2047
    uint32_t remainder = (voice->phase >> FRAC_SHIFT) & FRAC_MASK; // 0..255
    uint8_t table_weight = voice->voice_param.table_weight;
    uint32_t va = voice->voice_param.wavetable[index];    
    ...
    uint32_t vb = voice->voice_param.wavetable2[index];
    ...
    int64_t tmp = ((int64_t)(a1+a2+b1+b2) * (int64_t)voice->ramp.current);
